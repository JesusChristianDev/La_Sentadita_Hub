...
async function isDisabledByEmail(email: string): Promise<boolean> {
  ...
  // MVP: listUsers es paginado. Buscamos unas cuantas páginas.
  // (Con pocos usuarios, esto va rápido.)
  const perPage = 1000;
  ...
  for (let page = 1; page <= 10; page += 1) {
    ...
    // Si esta página vino incompleta, ya no hay más usuarios.
...
export async function login(formData: FormData) {
  ...
  // Si está desactivado en profiles => mensaje correcto.
  if (error || !data.user) {
